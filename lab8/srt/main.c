...
main( int argc, char **argv )
{
    ...
    // int spacing = width / nthreads;
    int thr_num[nthreads];
    ...
    {
	thr_num[t] = t;
	ret = pthread_create(&threads[t], NULL, rayT, &thr_num[t]);
	if(ret){
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            for( int xs=-halfSamples; xs<=halfSamples; ++xs )
            {
                for( int ys=-halfSamples; ys<=halfSamples; ++ys )
                {
                    ...
                    add( pixel_color[px][py], pixel_color[px][py], sample_color );
                }
            }
            ...
            if( halfSamples )
            {
                mul( pixel_color[px][py], pixel_color[px][py],
